...
public class RewardServiceImpl implements RewardService {
	...
	public List<Customer> calculateRewardsAll() {
		...
		if (!customerList.isEmpty()) {
			...
			for (Customer customer : customerList) {
				logger.debug("Getting customer transactions by Id and calculating rewards for customer "
						+ customer.getName());
...

	{
		...
		if (customer != null) {
			logger.debug(
					"Getting customer transactions by Id and calculating rewards for customer " + customer.getName());
			Set<Transaction> setOfTransaction = customer.getTransactions();
			setRewardsPerMonth(setOfTransaction, customer);
			logger.debug("End of reward calculation for customer " + customer.getName());
		}
	...
	private void setRewardsPerMonth(Set<Transaction> setOfTransaction, Customer customer) {
		...
		for (Transaction transaction : setOfTransaction) {
			...
			logger.debug("Calculating Rewards for customer" + customer.getName() + " for Transaction Id "
					+ transaction.getId());
			...
			if ((currentdate.getYear() == transactionYear) && (currentdate.getMonth().getValue() == transactionMonth))
				customer.setThirdMonthRewards(customer.getThirdMonthRewards()
						+ calculateRewardAmountPerTransaction(transaction.getTransaction_amount()));
			else if ((currentdate.minusMonths(1).getYear() == transactionYear)
					&& (currentdate.minusMonths(1).getMonth().getValue() == transactionMonth))
				customer.setSecondMonthRewards(customer.getSecondMonthRewards()
						+ calculateRewardAmountPerTransaction(transaction.getTransaction_amount()));
			else if ((currentdate.minusMonths(2).getYear() == transactionYear)
					&& (currentdate.minusMonths(2).getMonth().getValue() == transactionMonth))
				customer.setFirstMonthRewards(customer.getFirstMonthRewards()
						+ calculateRewardAmountPerTransaction(transaction.getTransaction_amount()));

			logger.debug("End of Calculating Rewards for customer" + customer.getName() + " for Transaction Id "
					+ transaction.getId());
